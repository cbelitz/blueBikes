...
# set global plot size options so they look better in Word (note to selves: We are not outputting to Word, so maybe we don't want to keep these global plot settings? I just remember knitr making half-sized graphics, so this bumped them up to be a bit larger)
...
**RQ1:** Can we predict bike rental usage by station, and by extension the positive/negative flow of bikes at that station?

**RQ2:** Can we interpret our results?

**RQ3:** Can we classify different stations based on their cluster patterns?
...
In Figure \ref{fig:figs} below, the stratified bands of color demonstrate how the K-means algorithm grouped these observations according to their activity level.
...
```{r figs, echo=FALSE, fig.width=7,fig.height=6,fig.cap="\\label{fig:figs}K Means"}
