...
### PREDICTING FLOW ###
pred.flow <- predict(lm_flow, ctrips.predictors[-train.v,])
pred.flow.MSE <- mean((pred.flow - ctrips.predictors$total.start[-train.v])^2)
pred.flow.MSE
...
# get training set
set.seed(1)
train = sample(1:nrow(clean_trips), nrow(clean_trips)*.75)
trips.test=clean_trips[-train ,"flow"]
