...
sprintf("Mean squared error on the test data for flow is %f",mean((yhat.rf.2-trips.test)^2)) #test MSE is 51, so on average off by ~11.5 (slightly better than initial model)
...
sprintf("Mean squared error on the test data for flow is %f", mean((yhat.rf-starts.test)^2)) #test MSE is 151, so on average off by 12 (better than SVM)
...
The best results for flow so far are superior to those found in the linear model. 
